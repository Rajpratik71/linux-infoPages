File: *manpages*,  Node: bridge,  Up: (dir)
...
SYNOPSIS
       ...
       bridge link set dev DEV  [ cost COST ] [ priority PRIO ] [ state STATE]
               ...
               hwmode { vepa | veb } ] [ mcast_flood { on | off } ] [ self ] [
               master ]

       bridge link [ show ] [ dev DEV ]

       bridge fdb { add | append | del | replace } LLADDR dev DEV { local |
               static | dynamic } [ self ] [ master ] [ router ] [ use ] [ dst
               IPADDR ] [ vni VNI ] [ port PORT ] [ via DEVICE ]
...
OPTIONS
       ...
       -json  Display results in JSON format. Currently available for vlan and
              fdb.
...
bridge link - bridge port
   ...
   bridge link set - set bridge specific attributes on a port
       ...
       mcast_flood on or mcast_flood off
              ...
              traffic for which there is no MDB entry. By default this flag is
              on.
...
bridge fdb - forwarding database management
   ...
   bridge fdb add - add a new fdb entry
       ...
       dev DEV
              ...
              self - the address is associated with the port drivers fdb. Usu‐
              ally hardware.


              master - the address is associated with master devices fdb. Usu‐
              ally software (default).
