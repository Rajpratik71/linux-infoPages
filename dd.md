...
`status=LEVEL'
     ...
     the `INFO' signal or when `dd' exits.  Specifying LEVEL will
     adjust the amount of information printed, with the last LEVEL
     specified taking precedence.
...
`conv=CONVERSION[,CONVERSION]...'
    ...
    `ebcdic'
          ...
          conversion.  This option implies `conv=block'; trailing
          spaces are added before being converted to EBCDIC.
...
error and then resume copying.   In the example below, `dd' is run in
the background to copy 5GB of data.  The `kill' command makes it output
