File: *manpages*,  Node: dmsetup,  Up: (dir)
...
SYNOPSIS
       ...
       dmsetup info -c|-C|--columns [--noheadings] [--separator separator]
              [-o fields] [-O|--sort sort_fields] [-S|--select Selection]
              [device_name]
...
OPTIONS
       ...
       --manglename {none|hex|auto}
              ...
              mangling  mode is one of: none (no mangling), hex (always do the
              mangling) and auto (only do the mangling if not mangled yet,  do
              nothing  if  already  mangled,  error  on mixed) Default mode is
              auto.   Character  whitelist:  0-9,  A-Z,  a-z,  #+-.:=@_.  This
...
COMMANDS
       ...
       create device_name  [-u uuid] [-n|--notable|--table <table>|table_file]
