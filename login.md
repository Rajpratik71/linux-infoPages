...
ATTRIBUTES
       ...
       └──────────┴───────────────┴──────────────────────┘
       In the above table, utent in race:utent signifies that if any of the functions setutent(3), getutent(3), or endutent(3) are used in  paral‐
       lel  in different threads of a program, then data races could occur.  login(3) and logout(3) calls those functions, so we use race:utent to
       remind users.
