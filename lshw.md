...
DESCRIPTION
       ...
       -disable test
              ...
              heuristics),  cpuinfo  (for  kernel-reported CPU detection), cpuid (for CPU detection), pci (for PCI/AGP access), isapnp (for ISA PnP extensions), pcmcia (for PCMCIA/PCCARD), ide (for IDE/AT‐
              API), usb (for USB devices),scsi (for SCSI) or network (for network interfaces detection).
