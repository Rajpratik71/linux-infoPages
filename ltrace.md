File: *manpages*,  Node: ltrace,  Up: (dir)
...
SYNOPSIS
       ...
       [-b|--no-signals] [-i] [-w|--where=nr] [-r|-t|-tt|-ttt] [-T] [-F  path‐
       list]  [-A  maxelts]  [-s  strsize] [-C|--demangle] [-a|--align column]
...
DESCRIPTION
       ...
       determine  what arguments each function has, it needs external declara‐
       tion of function prototypes.  Those are stored in files  called  proto‐
       type  libraries--see  ltrace.conf(5) for details on the syntax of these
       files.  See the section PROTOTYPE LIBRARY DISCOVERY to learn how ltrace
...
PROTOTYPE LIBRARY DISCOVERY
       ...
       type library corresponding to the library where the hit occured.   When
