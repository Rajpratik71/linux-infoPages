File: *manpages*,  Node: mksh,  Up: (dir)
...
AUTHORS
     ...
     Arnold Robbins, Lou Salkind, and others.  The first release of pdksh was
     ...
     and Michael Rendell <michael@cs.mun.ca>.  The effort of several projects,
...
CAVEATS
     ...
     64-bit integer arithmetics, use lksh (legacy mksh) instead, but be aware
     ...
     digit-zero numbers should be interpreted as octal.  set -o posix will
     cause the shell (either mksh or lksh) to behave more like the standard
     expects.

     For the purpose of POSIX, mksh supports only the “C” locale.  For users
     of UTF-8 locales, the following sh code makes the shell match the locale:
...
BUGS
     ...
     This document attempts to describe mksh R50e and up, with vendor patches
     from SUSE, compiled without any options impacting functionality, such as
     MKSH_SMALL, when not called as /bin/sh which, on some systems only,
     enables set -o posix or set -o sh automatically (whose behaviour differs
     across targets), for an operating environment supporting all of its
     advanced needs.
