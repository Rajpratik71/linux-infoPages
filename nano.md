...
specific line of a file by adding the line number with a plus sign
before the filename, and even in a specific column by adding it with a
comma.  So the complete command synopsis is:

     'nano [OPTION]... [[+LINE[,COLUMN]|+,COLUMN] FILE]...'

   But normallly you would set your preferred options in your '.nanorc'
file.  And when the 'positionlog' option is set (making 'nano' remember
the cursor position when you close a file), you will rarely need to
specify a line number.

Next: Editor Basics,  Prev: Invoking,  Up: Top
...
'--constantshow'
     ...
     statusbar.  Note that this overrides option '-U' ('--quickblank').
...
Next: Built-in Help,  Prev: Command-line Options,  Up: Top
...
Next: Commands,  Up: Editor Basics
...
edited.
   ...
   * For characters with a single-byte code, pressing the Esc key twice
     and then typing a three-digit decimal number (from 000 to 255) will
     enter the character with the corresponding value.
...
Next: The Cutbuffer,  Prev: Entering Text,  Up: Editor Basics
...
Next: The Mark,  Prev: Commands,  Up: Editor Basics
...
Next: Screen Layout,  Prev: The Cutbuffer,  Up: Editor Basics
...
Next: Search and Replace,  Prev: The Mark,  Up: Editor Basics
...
and two help lines.
   ...
   The titlebar consists of three sections: left, center and right.  The
section on the left displays the version of 'nano' being used.  The
...
Next: Using the Mouse,  Prev: Screen Layout,  Up: Editor Basics

4.6 Search and Replace
======================

One can search the current buffer for the occurrence of any string with
the Search command (default key binding: ^W). The default search mode is
forward, case-insensitive, and for literal strings.  But one can search
backwards by pressing M-B, search case sensitively with M-C, and
interpret regular expressions in the search string with M-R.

   A regular expression in a search string always covers just one line;
it cannot span multiple lines.  And when replacing (with ^\ or M-R) the
replacement string cannot contain a newline (LF).

Next: Limitation,  Prev: Search and Replace,  Up: Editor Basics
...
Prev: Using the Mouse,  Up: Editor Basics
...
Next: Feature Toggles,  Prev: Editor Basics,  Up: Top
...
and the available keystrokes.  Navigation is via the ^Y (Page Up) and ^V
...
invoked via Meta-key sequences (see *Note Commands::, for more info).
...
'Whitespace Display Toggle (Meta-P)'
     toggles the whitespace-display mode.  See *Note Whitespace::, for
     more info.
...
Next: The File Browser,  Prev: Feature Toggles,  Up: Top
...
highlighting and to rebind keys -- *Note Syntax Highlighting::, and
*Note Rebinding Keys::.  'nano' will read one command per line.
...
Next: Syntax Highlighting,  Up: Nanorc Files
...
Next: Rebinding Keys,  Prev: Settings,  Up: Nanorc Files
...
'syntax "str" ["fileregex" ...]'
     ...
     expression "fileregex".  All subsequent 'color', 'icolor', 'header'
     and other such statements will apply to this "str" syntax until a
...
'magic "regex" ...'
     Add one or more regexes which will be compared against the result
     of querying the magic database about the file to be edited, to
     determine whether this syntax should be used for that file.  This
     functionality only works when libmagic is installed on the system
     and will be silently ignored otherwise.
...
'color fgcolor,bgcolor start="sr" end="er"'
     ...
     "er" with foreground color "fgcolor" and background color
     "bgcolor", at least one of which must be specified.  This means
     that, after an initial instance of "sr", all text will be
     highlighted until the first instance of "er".  This allows syntax
...
'extendsyntax str directive [arg ...]'
     ...
     defined in one of the system-installed files (which are normally
     not writable).

Prev: Syntax Highlighting,  Up: Nanorc Files
...
Next: Pico Compatibility,  Prev: Nanorc Files,  Up: Top
...
Next: Building and Configure Options,  Prev: The File Browser,  Up: Top
...
'Search and Replace History'
     ...
     These strings can be accessed with the up/down arrow keys.  A
     retrieved string can subsequently be edited.
...
'Toggles'
     ...
     off.  See the internal help function (^G) for a list of features
     that can be toggled.  Or see *Note Feature Toggles::.  instead.
