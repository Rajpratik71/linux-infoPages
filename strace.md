File: *manpages*,  Node: strace,  Up: (dir)
...
OPTIONS
       ...
       -k          Print  the  execution stack trace of the traced pro‐
                   cesses after each system call (experimental).   This
                   option  is  available  only  if strace is built with
                   libunwind.
...
DIAGNOSTICS
       ...
       zero unless no processes has been attached or there was an unex‐
...
NOTES
       ...
       Some  system  calls  have different names in different architec‐
       tures and personalities.  In these cases, system call  filtering
       and printing uses the names that match corresponding __NR_* ker‐
       nel macros of the tracee's architecture and personality.   There
       are  two  exceptions from this general rule: arm_fadvise64_64(2)
       ARM syscall and xtensa_fadvise64_64(2) Xtensa syscall  are  fil‐
