File: *manpages*,  Node: systemctl,  Up: (dir)
...
COMMANDS
   ...
   Unit Commands
       ...
       set-property NAME ASSIGNMENT...
           ...
           such as resource control settings at runtime. The changes are
           applied instantly and stored as drop-in files on disk for future
           boots (See section "Overriding vendor settings" in systemd.unit(5)
           for details about drop-in files). If the option --runtime is
           passed, the drop-in creation is suppressed and the settings only
           apply until the next reboot. The syntax of the property assignment
           follows closely the syntax of assignments in unit files.

           Available properties are: CPUAccounting, CPUShares,
           StartupCPUShares, CPUQuota, MemoryAccounting, MemoryLimit,
           TasksAccounting, TasksMax, BlockIOAccounting, BlockIOWeight,
           StartupBlockIOWeight, BlockIODeviceWeight, BlockIOReadBandwidth,
           BlockIOWriteBandwidth, DeviceAllow, DevicePolicy, NetClass, Slice
           and Delegate (See systemd.resource-control(5) for details about
           these properties).
   ...
   Unit File Commands
       ...
       is-enabled NAME...
           ...
           ├──────────────────┤ through one or more     │           │
           │"linked-runtime"  │ symlinks to the unit    │           │
           │                  │ file (permanently in    │           │
           │                  │ /etc/systemd/system/    │           │
           │                  │ or transiently in       │ > 0       │
           │                  │ /run/systemd/system/),  │           │
