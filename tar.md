...
separating them or dashes preceding them(1).  This set of letters must
be the first to appear on the command line, after the 'tar' program name
...
confusing.  In the command 'tar cvbf 20 /dev/rmt0', for example, '20' is
the argument for '-b', '/dev/rmt0' is the argument for '-f', and '-v'
does not have a corresponding argument.  Even using short options like
in 'tar -c -v -b 20 -f /dev/rmt0' is clearer, putting all arguments next
...
announcing the short option style instead of the old option style; short
...
'-P'
     ...
     member names.  This option disables that behavior.  *Note
     absolute::.

'--acls'
     Causes 'tar' to store/restore/list POSIX ACL's.  *Note
     Attributes::.
...
'--level=N'
     Force incremental backup of level N.  As of GNU 'tar' version 1.26,
...
'--no-acls'
     Causes 'tar' not to store, extract or list POSIX ACL's.  *Note
     Attributes::.
...
'--no-selinux'
     Causes 'tar' not to store, extract or list SELinux security
     context.  *Note Attributes::.
...
'--no-xattrs'
     Causes 'tar' not to store, extract or list xattrs.  *Note
     Attributes::.
...
'--selinux'
     Causes 'tar' to store, extract or list SELinux security context.
     *Note Attributes::.
...
'--xattrs'
     Causes 'tar' to store, restore or list extended file attributes.
     For more info see *Note Attributes::.
...
--verbose' ('tar cfv -', or even 'tar cv'--if the installer let standard
...
other than POSIX).  The '%u' meta-character is replaced with the ordinal
number of the checkpoint.  Thus, the above example could produce the
following output when used with the '--create' option:
...
xattr-write
     '%s: Cannot set '%s' extended attribute for file '%s''
     '%s: Cannot set POSIX ACLs for file '%s''
     '%s: Cannot set SELinux context for file '%s''
